...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/09 16:57:14 by brbaazi           #+#    #+#             */
/*   Updated: 2024/12/13 10:10:45 by brbaazi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*savestr[OPEN_MAX];
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	savestr[fd] = ft_read(fd, savestr[fd]);
	if (!savestr[fd])
		return (NULL);
	line = newline(savestr[fd]);
	savestr[fd] = lastline(savestr[fd]);
	return (line);
}
